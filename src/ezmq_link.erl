...
connecting({long, IdLength}, State = #state{version = {_Major, _}}) ->
...
open({long, IdLength}, State = #state{version = {_Major, _}}) ->
    send_major(next_handshake_state('$major', State#state{remote_id_len = IdLength}));
...
handshake(_Msg, State = #state{hs_state = _HsState}) ->
    {stop, normal, State}.
...
handshake(_, '$major', <<_Major:8, _/binary>>, State) ->
    {stop, normal, State};
...
handshake(_Version, _HsState, _Data, State) ->
    {stop, normal, State}.
